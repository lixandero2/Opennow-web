...
class CacheManager {
  ...
  constructor() {
    this.cacheDir = join(resolve(process.env.OPENNOW_DATA_DIR ?? ".opennow-data"), CACHE_DIRECTORY);
  }
  ...
  async deleteAll(): Promise<void> {
    ...
    try {
      ...
      const thumbsDir = join(resolve(process.env.OPENNOW_DATA_DIR ?? ".opennow-data"), THUMBNAILS_DIRECTORY);
      try {
